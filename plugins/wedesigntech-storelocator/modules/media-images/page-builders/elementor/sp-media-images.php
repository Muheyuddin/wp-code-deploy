...
class DTStoreLocatorSpMediaImages extends Widget_Base {
	...
	public function get_name() {
		return 'dtsl-widget-sp-media-images';
	}

	public function get_title() {
		return esc_html__( 'Media - Images', 'dtsl' );
	}

	public function get_style_depends() {
		return array ('dtsl-media-images-frontend');
	}
	...
	protected function _register_controls() {
		...
		$this->start_controls_section( 'media_images_default_section', array(
			...
			$this->add_control( 'show_image_description', array(
				'label'       => esc_html__( 'Show Image Description', 'dtsl' ),
				'type'        => Controls_Manager::SELECT,
				'options'     => array(
					'false' => esc_html__('False', 'dtsl'),
					'true'  => esc_html__('True', 'dtsl'),
				),
				'description' => esc_html__('Choose "True" if you like to show image description in carousel.', 'dtsl'),
				'default'      => 'false'
			) );

			$this->add_control( 'include_featured_image', array(
				'label'       => esc_html__( 'Include Feature Image', 'dtsl' ),
				'type'        => Controls_Manager::SELECT,
				'options'     => array(
					'false' => esc_html__('False', 'dtsl'),
					'true'  => esc_html__('True', 'dtsl'),
				),
				'description' => esc_html__('Choose "True" if you like to include featured image in this gallery.', 'dtsl'),
				'default'      => 'false'
			) );
		...
		$this->start_controls_section( 'media_images_carousel_section', array(
			...
			$this->add_control( 'carousel_verticaldirection', array(
				'label'       => esc_html__('Enable Vertical Direction', 'dtsl'),
				'type'        => Controls_Manager::SELECT,
				'options'     => array(
					'false' => esc_html__('False', 'dtsl'),
					'true'  => esc_html__('True', 'dtsl'),
				),
				'description' => esc_html__( 'To make your slides to navigate vertically.', 'dtsl' ),
				'default'      => 'false'
			) );
			...
			$this->add_control( 'carousel_paginationtype', array(
				...
				'options'     => array(
					...
					'scrollbar'   => esc_html__('Scroll Bar', 'dtsl'),
					'thumbnail'   => esc_html__('Thumbnail', 'dtsl')
				),
			...
			$this->add_control( 'carousel_numberofthumbnails', array(
				'label'       => esc_html__('Number of Thumbnails', 'dtsl'),
				'type'        => Controls_Manager::SELECT,
				'options'     => array(
					3 => 3,
					4 => 4,
				...
				'description' => esc_html__( 'Number of thumbnails to show.', 'dtsl' ),
	...
	protected function render() {
		...
		echo do_shortcode('[dtsl_sp_media_images '.$attributes.' /]');
