...
function dtsl_dashboard_favourite_listings_page_content() {
	...
	$output .= '<div class="dtsl-dashbord-section-holder">';
		...
		$output .= '<div class="dtsl-dashbord-section-holder-intro">';
			$output .= '<div class="dtsl-dashbord-section-title">'.sprintf( esc_html__('Favourite %1$s', 'dtsl'), $listing_plural_label ).'</div>';
			$output .= '<div class="dtsl-dashbord-section-title-notes">'.sprintf( esc_html__('%1$s that you have marked as your favourite.', 'dtsl'), $listing_plural_label ).'</div>';
		$output .= '</div>';

		$output .= '<div class="dtsl-dashbord-section-holder-content dtsl-dashbord-load-favourite-listings-content">';
			$output .= dtsl_dashboard_favourite_listings_table_content();
		$output .= '</div>';
...
add_action( 'wp_ajax_dtsl_dashboard_favourite_listings_table_content', 'dtsl_dashboard_favourite_listings_table_content' );
add_action( 'wp_ajax_nopriv_dtsl_dashboard_favourite_listings_table_content', 'dtsl_dashboard_favourite_listings_table_content' );
function dtsl_dashboard_favourite_listings_table_content() {
	...
	$function_call = (isset($_REQUEST['function_call']) && $_REQUEST['function_call'] != '') ? dtsl_recursive_sanitize_text_field($_REQUEST['function_call']) : 'dtsl_dashboard_favourite_listings_table_content';
	$output_div = (isset($_REQUEST['output_div']) && $_REQUEST['output_div'] != '') ? dtsl_recursive_sanitize_text_field($_REQUEST['output_div']) : 'dtsl-dashbord-load-favourite-listings-content';
							...
							foreach($favourite_items_filtered as $package_listing_id) {
								$output .= '<tr>
												...
												<td>
									                ...
									                <a data-tooltip="'.sprintf( esc_html__( 'Remove Favourite %1$s', 'dtsl' ), $dt_sl_listing_singular_label ).'" class="dtsl-remove-favourite-listing" data-nonce="'.wp_create_nonce('dtsl-remove-favourite-listing-'. $package_listing_id ).'"  data-listing-id="'.esc_attr($package_listing_id).'" data-user-id="'.esc_attr($user_id).'" data-dashboard-page-url="'.esc_url(get_permalink($dashboard_page_id)).'">
						                				<i class="far fa-times-circle"></i>
						                			</a>

				                					<a data-tooltip="'.sprintf( esc_html__( 'View Favourite %1$s', 'dtsl' ), $dt_sl_listing_singular_label ).'" href="'.get_permalink($package_listing_id).'">
...
add_action( 'wp_ajax_dtsl_remove_favourite_listing', 'dtsl_remove_favourite_listing' );
add_action( 'wp_ajax_nopriv_dtsl_remove_favourite_listing', 'dtsl_remove_favourite_listing' );
function dtsl_remove_favourite_listing() {
	...
	if(wp_verify_nonce($nonce, 'dtsl-remove-favourite-listing-'.$listing_id)) {
