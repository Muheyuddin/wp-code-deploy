<?php
add_action( 'vc_before_init', 'dtsl_sf_submit_button_vc_map' );

function dtsl_sf_submit_button_vc_map() {

	$dt_sl_listing_singular_label = apply_filters( 'dt_sl_listing_label', 'singular' );

	vc_map( array(
		"name" => esc_html__( 'Submit Button', 'dtsl' ),
		"base" => "dtsl_sf_submit_button",
		"icon" => "dtsl_sf_submit_button",
		"category" => DTSL_PB_MODULE_SEARCHFORM_TITLE,
		"params" => array(

			// Output Type
			array(
				'type' => 'dropdown',
				'heading' => esc_html__('Output Type','dtsl'),
				'param_name' => 'output_type',
				'value' => array(
					esc_html__('Same Page - Ajax Load', 'dtsl') => '',
					esc_html__('Separate Page', 'dtsl') => 'separate-page',
				),
				'description' => esc_html__( 'Choose how you like to display search output.', 'dtsl' ),
				'std' => '',
				'edit_field_class' => 'vc_column vc_col-sm-6',
				'admin_label' => true
			),

			// Separate Page URL
			array(
				'type' => 'textfield',
				'heading' => esc_html__( 'Separate Page URL', 'dtsl' ),
				'param_name' => 'separate_page_url',
				'description' => esc_html__( 'Separate page url in which search content have to displayed. You have to create that page with search form shortcode.', 'dtsl' ),
				'dependency' => array( 'element' => 'output_type', 'value' => 'separate-page'),
				'edit_field_class' => 'vc_column vc_col-sm-6'
			),

			// Class
			array(
				'type' => 'textfield',
				'heading' => esc_html__( 'Class', 'dtsl' ),
				'param_name' => 'class',
				'description' => esc_html__( 'If you wish you can add additional class name here.', 'dtsl' ),
				'edit_field_class' => 'vc_column vc_col-sm-6'
			),

		)
	) );
}
?>