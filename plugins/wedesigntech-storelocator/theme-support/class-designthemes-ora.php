...
if ( ! class_exists( 'DTStoreLocatorDesignThemes' ) ) {
	...
	class DTStoreLocatorDesignThemes {
		...
		function __construct() {
			...
			add_filter( 'ora_header_footer_default_cpt',  array ( $this, 'dtsl_dt_header_footer_default_cpt' ) );

			add_action( 'dtsl_before_main_content',  array ( $this, 'dtsl_dt_before_main_content' ), 10 );
			add_action( 'dtsl_after_main_content',  array ( $this, 'dtsl_dt_after_main_content' ), 10 );
			...
			add_filter( 'cs_metabox_options',  array ( $this, 'dtsl_cs_metabox_options'), 1 );

		}

		function dtsl_dt_header_footer_default_cpt( $custom_posts ) {

			$custom_posts[] = 'dtsl_listings';
			$custom_posts[] = 'dtsl_packages';

			return $custom_posts;

		}

		function dtsl_dt_before_main_content() {

			if (is_singular( 'dtsl_listings' ) || is_singular( 'dtsl_packages' )) {

				global $post;
				$post_id = $post->ID;

			    $settings = get_post_meta($post_id, 'dtsl_default_settings', true);
				...
				<div id="header-wrapper" class="<?php echo esc_attr($header_class); ?>">
			        ...
			        if( !empty( $global_breadcrumb ) ) {
			            ...
			            if(isset($settings['enable-sub-title']) && $settings['enable-sub-title']) {
			                ...
			                $bstyle = ora_cs_get_option( 'breadcrumb-style', 'default' );
			                ...
			                if(is_singular( 'dtsl_listings' )) {
			                	...
			                	$breadcrumbs[] = '<a href="'.esc_url(get_post_type_archive_link('dtsl_listings')).'">'.$listing_plural_label.'</a>';
			                }

			                $breadcrumbs[] = the_title( '<span class="current">', '</span>', false );
			                $bcsettings = isset( $settings['breadcrumb_background'] ) ? $settings['breadcrumb_background'] :  array ();
			                $style = ora_breadcrumb_css($bcsettings);

			                ora_breadcrumb_output ( the_title( '<h1>', '</h1>',false ), $breadcrumbs, $bstyle, $style );
			...
			if(is_post_type_archive('dtsl_listings') || is_tax('dtsl_listings_category') || is_tax('dtsl_listings_city') || is_tax('dtsl_listings_neighborhood') || is_tax('dtsl_listings_countystate') || is_tax('dtsl_listings_ctype') || is_tax('dtsl_listings_amenity') || is_post_type_archive('dtsl_packages') || is_author()) {
				...
				<div id="header-wrapper" class="<?php echo esc_attr($header_class); ?>">
				    ...
				    if( !empty( $global_breadcrumb ) ) {
				        ...
				        ora_breadcrumb_output ( $title, $breadcrumbs, $bstyle, $style );
		...
		function dtsl_cs_metabox_options( $options ) {
			...
			$options[] = array (
				'id'        => 'dtsl_default_settings',
				'title'     => esc_html__('Page Settings', 'dtsl'),
				'post_type' => array ( 'dtsl_listings', 'dtsl_packages' ),
				'context'   => 'normal',
				'priority'  => 'high',
				'sections'  => array (
