<?php
add_action( 'vc_before_init', 'dtsl_sp_contact_form_vc_map' );

function dtsl_sp_contact_form_vc_map() {

	$dt_sl_listing_singular_label = apply_filters( 'dt_sl_listing_label', 'singular' );
	$incharge_singular_label = apply_filters( 'dt_sl_incharge_label', 'singular' );

	vc_map( array(
		"name" => esc_html__( 'Contact Form', 'dtsl' ),
		"base" => "dtsl_sp_contact_form",
		"icon" => "dtsl_sp_contact_form",
		...
		"params" => array(
			...
			array(
				...
				'description' => sprintf( esc_html__('Provide %1$s id for which you have to contact form. No need to provide ID if it is used in %1$s single page.', 'dtsl'), strtolower($dt_sl_listing_singular_label) ),
				'admin_label' => true,
			...
			array(
				'type' => 'textfield',
				'heading' => esc_html__( 'Textarea Placeholder', 'dtsl' ),
			...
			array(
				'type' => 'textfield',
				'heading' => esc_html__( 'Submit Button Label', 'dtsl' ),
			...
			array(
				'type' => 'dropdown',
				'heading' => esc_html__('Contact Point','dtsl'),
				'param_name' => 'contact_point',
				'value' => array(
					sprintf( esc_html__( '%1$s Email', 'dtsl' ), $dt_sl_listing_singular_label ) => '',
					esc_html__('Author Email', 'dtsl') => 'author-email',
					sprintf( esc_html__('%1$s Email', 'dtsl'), $incharge_singular_label ) => 'incharge-email',
				),
			...
			array(
				'type' => 'dropdown',
				'heading' => esc_html__('Include Admin', 'dtsl'),
