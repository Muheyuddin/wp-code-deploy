...
function dtdr_save_options_settings() {
	...
	$dtdr_settings[$settings] = dtdr_recursive_sanitize_text_field($_REQUEST['dtdr'][$settings]);
	...
	if (get_option('dtdr-settings') != $dtdr_settings) {
		if (update_option('dtdr-settings', $dtdr_settings)) {
			echo esc_html__('Options have been updated successfully!','dtdr');
		}
	} else {
		echo esc_html__('No changes done!','dtdr');
	}
...
function dtdr_save_skin_settings() {
	...
	$dtdr_skin_settings = dtdr_recursive_sanitize_text_field($_REQUEST['dtdr-skin-settings']);

	update_option('dtdr-skin-settings', $dtdr_skin_settings);

	echo esc_html__('"Skin" settings have been updated successfully!', 'dtdr');
...
function dtdr_process_imported_file() {
	...
	require_once DTDR_PLUGIN_PATH . 'settings/simplexlsx.class.php';

	$import_file = dtdr_recursive_sanitize_text_field($_REQUEST['import_file']);
	$import_id = dtdr_recursive_sanitize_text_field($_REQUEST['import_id']);

	if($import_id != '') {
		$attachment_url = get_attached_file($import_id, false);
	} else {
		$attachment_url = DTDR_PLUGIN_PATH . 'import/listings.xlsx';
	}
