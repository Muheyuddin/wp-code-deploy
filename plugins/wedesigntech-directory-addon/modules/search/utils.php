...
if(!function_exists('dtdr_search_listings_content')) {
	function dtdr_search_listings_content() {
		...
		$output .= '<div class="dtdr-search-container dtdr-search-listings-container">';
			...
			$output .= '<select class="dtdr-search-listings-seller dtdr-chosen-select" name="dtdr-search-listings-seller" data-placeholder="'.sprintf( esc_html__('Choose %1$s ...', 'dtdr'), $seller_singular_label ).'" class="dtdr-chosen-select">';
...
if(!function_exists('dtdr_search_sellerwise_listings')) {
	function dtdr_search_sellerwise_listings() {
		...
		$output .= '<div class="dtdr-column dtdr-one-half first">';
				...
				$output .= '<table border="0" cellpadding="0" cellspacing="0" class="dtdr-custom-table">
								...
								<tbody class="dtdr-custom-table-content">';
									...
									if ( $listings_query->have_posts() ) :
										...
										$output .= '<tr>
														<td colspan="5">'.esc_html__('No Records Found!', 'dtdr').'</td>
													</tr>';
			...
			$output .= '<div class="dtdr-search-listings-count">'.sprintf( esc_html__( 'Total %1$s', 'dtdr' ), $listing_plural_label ).'<span>'.$listings_query->found_posts.'</span></div>';
										...
										$output .= '<tr>
														...
														<td>';
															...
															if($seller_incharges > 0) {
																$output .= '<a href="#" class="custom-button-style dtdr-search-seller-incharges" data-sellerid="'.esc_html($seller_id).'">'.esc_html__('View Details', 'dtdr').'</a>';
															}
														...
														$output .= '</td><td>';
															...
															$output .= $post_cnt;
															if($post_cnt > 0) {
																$output .='<a href="#" class="custom-button-style dtdr-search-seller-listings" data-sellerid="'.esc_html($seller_id).'">'.esc_html__('View Details', 'dtdr').'</a>';
															}
...
if(!function_exists('dtdr_search_packages')) {
	function dtdr_search_packages() {
		...
		$output .= '<div class="dtdr-custom-table-wrapper">';
										...
										$output .= '<tr>
														...
														<td>';
															...
															if(count($purchased_users) > 0) {
																$output .='<a href="#" class="custom-button-style dtdr-search-package-purchases" data-packageid="'.esc_html($package_id).'">'.esc_html__('View Details', 'dtdr').'</a>';
															}
			...
			$output .= '<div class="dtdr-search-packages-count">'.esc_html__( 'Total Packages', 'dtdr' ).'<span>'.$packages_query->found_posts.'</span></div>';
