...
if (!class_exists ( 'DTDirectoryRegisterMediaImagesModule' )) {

	class DTDirectoryRegisterMediaImagesModule extends DTDirectoryAddon {
		...
		function __construct() {
			...
			$this->dtdr_define_constants( 'DTDR_MIMAGES_PLUGIN_PATH', DTDR_PLUGIN_PATH . 'modules/media-images/' );
			$this->dtdr_define_constants( 'DTDR_MIMAGES_PLUGIN_URL', DTDR_PLUGIN_URL . 'modules/media-images/' );
		...
		function dtdr_metabox_tabs_tab($tabs) {
			...
			$tabs['media-images'] = array (
				'label' => esc_html__('Media - Images', 'dtdr'),
		...
		function dtdr_register_dependent_files() {
			...
			wp_register_style ( 'dtdr-media-images-frontend', DTDR_MIMAGES_PLUGIN_URL . 'assets/media-images-frontend.css', array ( 'fontawesome', 'material-icon', 'dtdr-base', 'dtdr-common', 'swiper' ) );
...
if( !function_exists('dtdrMediaImagesModule') ) {
	function dtdrMediaImagesModule() {
		return DTDirectoryRegisterMediaImagesModule::instance();
	}
}
