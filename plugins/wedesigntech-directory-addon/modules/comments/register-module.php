...
if (!class_exists ( 'DTDirectoryRegisterCommentsModule' )) {

	class DTDirectoryRegisterCommentsModule extends DTDirectoryAddon {
		...
		function __construct() {
			...
			$this->dtdr_define_constants( 'DTDR_COMMENTS_PLUGIN_PATH', DTDR_PLUGIN_PATH . 'modules/comments/' );
			$this->dtdr_define_constants( 'DTDR_COMMENTS_PLUGIN_URL', DTDR_PLUGIN_URL . 'modules/comments/' );
		...
		function dtdr_register_dependent_files() {
			...
			wp_register_style ( 'dtdr-comments-frontend', DTDR_COMMENTS_PLUGIN_URL . 'assets/comments-frontend.css', array ( 'fontawesome', 'material-icon', 'dtdr-base', 'dtdr-common', 'dtdr-modules-singlepage' ) );
...
if( !function_exists('dtdrCommentsModule') ) {
	function dtdrCommentsModule() {
		return DTDirectoryRegisterCommentsModule::instance();
	}
}
