...
if(!function_exists('dtdr_listing_attachments_field')) {
    function dtdr_listing_attachments_field($item_id) {
        ...
        $output .= '<div class="dtdr-attachments-box-container">';
                    ...
                    foreach($dtdr_media_attachments_titles as $dtdr_media_attachments_title) {
                        ...
                        $output .= '<div class="dtdr-attachments-box-item">
                                        <div class="dtdr-column dtdr-one-column first">
                                            <input name="dtdr_media_attachments_titles[]" class="dtdr_media_attachments_titles" type="text" value="'.esc_attr($dtdr_media_attachments_title).'" placeholder="'.esc_html__('Title', 'dtdr').'" />
                                        </div>
                                        <div class="dtdr-column dtdr-one-column first dtdr-upload-media-items-container">
                                            <input name="dtdr_media_attachments_items_url" type="text" value="'.esc_url($attachment_url).'" placeholder="'.esc_html__('Item', 'dtdr').'" class="uploadfieldurl" readonly />
                                            <input name="dtdr_media_attachments_items[]" type="hidden" value="'.esc_attr($dtdr_media_attachments_items[$j]).'" placeholder="'.esc_html__('Item', 'dtdr').'" class="uploadfieldid" readonly />
            ...
            $output .= '<a href="#" class="dtdr-add-attachments-box custom-button-style">'.esc_html__('Add Attachment', 'dtdr').'</a>';

            $output .= '<div class="dtdr-attachments-box-item-toclone hidden">
                            <div class="dtdr-column dtdr-one-column first">
                                <input id="dtdr_media_attachments_titles" type="text" placeholder="'.esc_html__('Title', 'dtdr').'" />
                            </div>
                            <div class="dtdr-column dtdr-one-column first dtdr-upload-media-items-container">
                                <input name="dtdr_media_attachments_items_url" type="text" placeholder="'.esc_html__('Item', 'dtdr').'" class="uploadfieldurl" readonly />
                                <input id="dtdr_media_attachments_items" type="hidden" placeholder="'.esc_html__('Item', 'dtdr').'" class="uploadfieldid" readonly />
                                <input type="button" value="'.esc_html__('Upload', 'dtdr').'" class="dtdr-upload-media-item-button show-preview" />
                                <input type="button" value="'.esc_html__('Remove', 'dtdr').'" class="dtdr-upload-media-item-reset" />
                            </div>
                            <div class="dtdr-attachments-box-options">
