...
class DTDirectorySpAverageRating extends Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'Average Rating', 'dtdr' );
	}

	public function get_style_depends() {
		$file_handlers =  dtdr_dependent_files_instance()->dtdr_single_page_module_files( 'dtdr_sp_average_rating' );
	...
	protected function register_controls() {
		...
		$this->start_controls_section( 'average_rating_default_section', array(
			...
			$this->add_control( 'display', array(
				'label'           => esc_html__( 'Display', 'dtdr' ),
				'type'            => Controls_Manager::SELECT,
				'options'         => array(
					'both'           => esc_html__('Both', 'dtdr'),
					'star-rating'    => esc_html__('Star Rating', 'dtdr'),
					'overall-rating' => esc_html__('Overall Rating', 'dtdr'),
				),
				'description'     => esc_html__( 'Choose type of content to display.', 'dtdr' ),
				'default'         => 'both',
			) );
