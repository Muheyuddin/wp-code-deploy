<?php
add_action( 'vc_before_init', 'dtdr_sp_mls_number_vc_map' );

function dtdr_sp_mls_number_vc_map() {

	$listing_singular_label = apply_filters( 'listing_label', 'singular' );

	vc_map( array(
		"name" => esc_html__( 'MLS Number', 'dtdr' ),
		"base" => "dtdr_sp_mls_number",
		"icon" => "dtdr_sp_mls_number",
		...
		"params" => array(
			...
			array(
				...
				'description' => sprintf( esc_html__('Provide %1$s id for which you have to display MLS number. No need to provide ID if it is used in %1$s single page.', 'dtdr'), strtolower($listing_singular_label) ),
