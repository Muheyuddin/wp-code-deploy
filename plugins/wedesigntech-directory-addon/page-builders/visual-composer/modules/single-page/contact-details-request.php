...
function dtdr_sp_contact_details_request_btn_vc_map() {
	...
	vc_map( array(
		"name" => esc_html__( 'Contact Details - Send Request Button', 'dtdr' ),
		...
		"params" => array(
			...
			array(
				...
				'description' => sprintf( esc_html__('Provide %1$s id for which you have to display contact details send request button. No need to provide ID if it is used in %1$s single page.', 'dtdr'), strtolower($listing_singular_label) ),
				'admin_label' => true
			),
			...
			array(
				'type' => 'textfield',
				'heading' => esc_html__( 'Button Label', 'dtdr' ),
