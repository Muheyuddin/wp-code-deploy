...
if(!function_exists('dtdr_login_form')) {
	function dtdr_login_form() {
		...
		$redirect_url = (isset($_REQUEST['redirect_url']) && $_REQUEST['redirect_url'] != '') ? dtdr_recursive_sanitize_text_field($_REQUEST['redirect_url']) : home_url('/');
		...
		$output = '<div class="dtdr-login-form-container">';
			...
			$output .= '<div class="dtdr-login-form">';
				...
				$output .= '<div class="dtdr-login-form-holder">';
					...
					$output .= '<div class="dtdr-title dtdr-login-title"><h2><span>'.esc_html__('Welcome!', 'dtdr').'<strong>'.esc_html__('Login', 'dtdr').'</strong></span></h2></div>';
					$output .= wp_login_form(array ('redirect' => $redirect_url, 'echo' => false));
		    		$output .= '<p class="tpl-forget-pwd"><a href="'.wp_lostpassword_url( get_permalink() ).'">'.esc_html__('Forgot password ?','dtdr').'</a></p>';
