<?php
if( !class_exists('DTDirectoryShortcodes') ) {

	class DTDirectoryShortcodes {
		...
		function __construct() {
			...
			add_shortcode ( 'dtdr_sp_comments_count', array( $this, 'dtdr_sp_comments_count' ) );
		...
		function dtdr_login_logout_links( $attrs, $content = null ) {
			...
			if(is_user_logged_in()) {
				...
				$output .= '<ul class="dtdr-custom-login '.esc_attr($attrs['class']).'">';
					...
					$output .= '<li><a href="'.wp_logout_url(home_url('/')).'" title="'.esc_html__('Logout', 'dtdr').'">'.esc_html__('Logout', 'dtdr').'</a></li>';
				...
				$output .= '<ul class="dtdr-custom-login '.esc_attr($attrs['class']).'">';
					...
					$output .= '<li><a href="#" title="'.esc_html__('Login', 'dtdr').'" class="dtdr-login-link" onclick="return false">'.esc_html__('Login', 'dtdr').'</a></li>';

					$output .= '<li><a href="'.wp_registration_url().'" title="'.esc_html__('Register', 'dtdr').'">'.esc_html__('Register', 'dtdr').'</a></li>';
		...
		function dtdr_listings_listing($attrs, $content = null) {
			...
			array_push($data_attributes, 'data-enableratings="'.esc_attr($attrs['enable_ratings']).'"');
			array_push($data_attributes, 'data-enablecommentscount="'.esc_attr($attrs['enable_comments_count']).'"');
						...
						$output .= '<div class="dtdr-swiper-pagination-holder">';
							...
							if($attrs['carousel_arrowpagination'] == 'true') {
								$output .= '<div class="dtdr-swiper-arrow-pagination">';
									$output .= '<a href="javascript:void(0);" class="dtdr-swiper-arrow-prev dtdr-swiper-arrow-prev-'.$attrs['module_id'].'">'.esc_html__('Prev', 'dtdr').'</a>';
									$output .= '<a href="javascript:void(0);" class="dtdr-swiper-arrow-next dtdr-swiper-arrow-next-'.$attrs['module_id'].'">'.esc_html__('Next', 'dtdr').'</a>';
								$output .= '</div>';
							}
				...
				foreach( $categories as $category ) {
						...
						if($cat_listings_id > 0) {
							...
							if(isset($price_label['sale_price']) && !empty($price_label['sale_price'])) {
								...
								$starting_price_html .= '<div class="dtdr-listing-taxonomy-starting-price">';
									$starting_price_html .= sprintf(esc_html__('Starts from %1$s', 'dtdr'), $_sale_price_label_html);
						...
						$output .= '<div class="dtdr-listing-taxonomy-item type1 '.$column_class.' '.$first_class.' '.$attrs['class'].'">';
							$output .= '<div class="dtdr-listing-taxonomy-icon-image">';
								if($attrs['media_type'] == 'icon') {
									...
									$output .= '<img src="'.$icon_image_url.'" alt="'.sprintf( esc_html__('%1$s Taxonomy Icon Image', 'dtdr'), $listing_singular_label ).'" title="'.sprintf( esc_html__('%1$s Taxonomy Icon Image', 'dtdr'), $listing_singular_label ).'" />';
								} else {
									$output .= '<img src="'.$image_url.'" alt="'.sprintf( esc_html__('%1$s Taxonomy Image', 'dtdr'), $listing_singular_label ).'" title="'.sprintf( esc_html__('%1$s Taxonomy Image', 'dtdr'), $listing_singular_label ).'" />';
								}
							$output .= '</div>';
							$output .= '<div class="dtdr-listing-taxonomy-meta-data">';
								...
								$output .= '<div class="dtdr-category-total-items"><span>'.$category->count.'</span> '.sprintf( esc_html__('%1$s', 'dtdr'), $listing_plural_label ).'</div>';
							$output .= '</div>';
						$output .= '</div>';
					...
					if($attrs['type'] == 'type5') {
						...
						$output .= '<div class="dtdr-listing-taxonomy-item type5 '.$column_class.' '.$first_class.' '.$attrs['class'].'">';
							...
							$output .= '<div class="dtdr-listing-taxonomy-meta-data">';
								$output .= '<h3><a href="'.get_term_link($category->term_id).'">'.esc_html($category->cat_name).'</a></h3>';
								$output .= '<div class="dtdr-category-total-items" '.$background_style.'><a href="'.get_term_link($category->term_id).'"><span>'.$category->count.'</span> '.sprintf( esc_html__('%1$s', 'dtdr'), $listing_plural_label ).'</a></div>';
							$output .= '</div>';
						$output .= '</div>';
					...
					if($attrs['type'] == 'type6') {
						...
						$output .= '<div class="dtdr-listing-taxonomy-item type6 '.$column_class.' '.$first_class.' '.$attrs['class'].'">';
							...
							$output .= '<div class="dtdr-listing-taxonomy-meta-data">';
								...
								$output .= '<h3><a href="'.get_term_link($category->term_id).'">'.esc_html($category->cat_name).'</a></h3>';
								$output .= '<div class="dtdr-category-total-items"><a href="'.get_term_link($category->term_id).'"><span>'.$category->count.'</span> '.sprintf( esc_html__('%1$s', 'dtdr'), $listing_plural_label ).'</a></div>';
							$output .= '</div>';
						$output .= '</div>';
		...
		function dtdr_sp_comments_count($attrs, $content = null) {
			$attrs = shortcode_atts ( array (
			...
			if($attrs['listing_id'] != '') {
				$output .= '<div class="dtdr-listings-comment-count-container '.$attrs['type'].' '.$attrs['class'].'">';
					$comments = get_approved_comments( $attrs['listing_id'] );
					$total_comments = count($comments);

					$dtdr_reviews_count_html = '<div class="dtdr-listings-average-rating-reviews-count">'.sprintf(esc_html__('%1$s reviews', 'dtdr'), $total_comments).'</div>';
	...
	DTDirectoryShortcodes::instance();
