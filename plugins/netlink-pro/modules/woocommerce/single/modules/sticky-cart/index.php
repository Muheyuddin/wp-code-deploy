...
if( !class_exists( 'Netlink_Shop_Single_Module_Sticky_Cart' ) ) {

    class Netlink_Shop_Single_Module_Sticky_Cart {
        ...
        function __construct() {
            ...
            // CSS
                add_filter( 'netlink_woo_css', array( $this, 'woo_css'), 10, 1 );

            // JS
                add_filter( 'netlink_woo_js', array( $this, 'woo_js'), 10, 1 );
        ...
        */
            ...
            function module_dir_path() {
                ...
                if( netlink_is_file_in_theme( __FILE__ ) ) {
                    return NETLINK_MODULE_DIR . '/woocommerce/single/modules/sticky-cart/';
                } else {
            ...
            function module_dir_url() {
                ...
                if( netlink_is_file_in_theme( __FILE__ ) ) {
                    return NETLINK_MODULE_URI . '/woocommerce/single/modules/sticky-cart/';
                } else {
                    ...
                    echo '<div class="container">';
                        echo '<div class="wdt-shop-single-sticky-addtocart-content">';
                            ...
                            echo '<div class="wdt-shop-single-sticky-addtocart-info">';
                                echo '<h3>'.netlink_html_output($product->get_name()).'</h3>';
                                echo wc_get_rating_html( $product->get_average_rating() );
                            echo '</div>';
                        echo '</div>';
                        echo '<div class="wdt-shop-single-sticky-addtocart-section">';
                            echo '<span class="wdt-shop-single-sticky-addtocart-price">'.netlink_html_output($product->get_price_html()).'</span>';
                            if(netlink_check_item_is_in_cart( $product->get_id() )) {
                                echo '<span class="wdt-shop-single-sticky-addtocart-added">'.esc_html__('Product Added To Cart', 'netlink-pro').'</span>';
                            } else {
...
if( !function_exists('netlink_shop_single_module_sticky_cart') ) {
	function netlink_shop_single_module_sticky_cart() {
		return Netlink_Shop_Single_Module_Sticky_Cart::instance();
	}
}

netlink_shop_single_module_sticky_cart();