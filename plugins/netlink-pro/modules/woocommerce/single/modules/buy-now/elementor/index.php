...
class Netlink_Shop_Elementor_Single_Buy_Now_Widgets {
	...
	function __construct() {
		...
		$this->netlink_shop_load_bn_modules();
	 ...
	 * Init
	 */
	function netlink_shop_load_bn_modules() {

		require netlink_shop_single_module_buy_now()->module_dir_path() . 'elementor/utils.php';
	...
	function netlink_shop_register_widget_styles( $suffix ) {
		...
		wp_register_style( 'wdt-shop-buy-now',
			netlink_shop_single_module_buy_now()->module_dir_url() . 'assets/css/style'.$suffix.'.css',
			array()
		);
	...
	function netlink_shop_register_widget_scripts( $suffix ) {
		...
		wp_register_script( 'wdt-shop-buy-now',
			netlink_shop_single_module_buy_now()->module_dir_url() . 'assets/js/scripts'.$suffix.'.js',
			array( 'jquery' ),
			false,
			true
		);
...
Netlink_Shop_Elementor_Single_Buy_Now_Widgets::instance();