...
class Netlink_Customize_Control_Typography extends WP_Customize_Control {
	...
	public function enqueue() {
		...
		wp_enqueue_script( 'netlink-plus-typography-control', NETLINK_PLUS_DIR_URL.'customizer/controls/typography/typography.js', array( 'jquery', 'customize-base' ), NETLINK_PLUS_VERSION, true );
		wp_enqueue_style( 'netlink-plus-typography-control',  NETLINK_PLUS_DIR_URL.'customizer/controls/typography/typography.css', null, NETLINK_PLUS_VERSION );

		$typo_localize = array(
			'inherit' => esc_html__( 'Inherit', 'netlink-plus' ),
			'100'     => esc_html__( 'Thin 100', 'netlink-plus' ),
			'200'     => esc_html__( 'Extra-Light 200', 'netlink-plus' ),
			'300'     => esc_html__( 'Light 300', 'netlink-plus' ),
			'400'     => esc_html__( 'Normal 400', 'netlink-plus' ),
			'500'     => esc_html__( 'Medium 500', 'netlink-plus' ),
			'600'     => esc_html__( 'Semi-Bold 600', 'netlink-plus' ),
			'700'     => esc_html__( 'Bold 700', 'netlink-plus' ),
			'800'     => esc_html__( 'Extra-Bold 800', 'netlink-plus' ),
			'900'     => esc_html__( 'Ultra-Bold 900', 'netlink-plus' ),
			'normal'  => esc_html__( 'Normal', 'netlink-plus' ),
			'normal'  => esc_html__( 'Normal', 'netlink-plus' ),
			'bold'    => esc_html__( 'Bold', 'netlink-plus' ),
			''        => esc_html__('Default', 'netlink-plus' ),
		);

		wp_localize_script( 'netlink-plus-typography-control', 'wdtPlusTypoObject', $typo_localize );
	}
	...
	public function get_custom_fonts() {
		$fonts = apply_filters( 'netlink_customizer_custom_fonts', array() );
		return $fonts;
	}
	...
	public function get_google_fonts() {
		...
		$google_fonts_file = NETLINK_PLUS_DIR_PATH . 'customizer/controls/typography/google-fonts.json';
	...
	public function to_json() {
		...
		if ( ! is_array( $choices ) || empty( $choices ) ) {
			...
			$this->choices = array(
				'font_family'     => esc_html__( 'Font Family', 'netlink-plus'),
				'font_weight'     => esc_html__( 'Font Weight', 'netlink-plus'),
				'text_transform'  => esc_html__( 'Text Transform', 'netlink-plus'),
				'text_align'      => esc_html__( 'Text Align', 'netlink-plus'),
				'text_decoration' => esc_html__( 'Text Decoration', 'netlink-plus'),
				'font_style'      => esc_html__( 'Font Style', 'netlink-plus'),
				'font_size'       => esc_html__( 'Font Size', 'netlink-plus'),
				'line_height'     => esc_html__( 'Line Height', 'netlink-plus'),
				'letter_spacing'  => esc_html__( 'Letter Spacing', 'netlink-plus'),
			);
		}
		...
		<div class="wrapper">
			...
			<div class="full-width">
				...
				<select class="font-family">
					<option value=""><?php esc_html_e( 'Inherit', 'netlink-plus' );?></option>
					<optgroup label="<?php esc_html_e( 'Other System Fonts', 'netlink-plus' );?>">
					...
					<# if( _.isEqual( data.has_custom_fonts, {'item':'yes' } ) ) { #>
						<optgroup label="<?php esc_html_e( 'Custom Fonts', 'netlink-plus' );?>">
					...
					<optgroup label="<?php esc_html_e( 'Google', 'netlink-plus' );?>">
				...
				<# if( !_.isUndefined( data.choices['font_weight'] ) ) { #>
					<div class="one-half">
						...
						<select class="font-weight">
							<option value="inherit"><?php esc_html_e( 'Inherit', 'netlink-plus' );?></option>
						</select>
					</div>
				<# } #>
				...
				<# if( !_.isUndefined( data.choices['font_style'] ) ) { #>
					<div class="one-half">
						...
						<select class="font-style">
							...
							<option value="normal"<# if ( 'none' === data.value['font-style'] ) { #>selected<# } #>><?php esc_attr_e( 'Normal', 'netlink-plus' ); ?></option>
							<option value="italic"<# if ( 'italic' === data.value['font-style'] ) { #>selected<# } #>><?php esc_attr_e( 'Italic', 'netlink-plus' ); ?></option>
							<option value="oblique"<# if ( 'oblique' === data.value['font-style'] ) { #>selected<# } #>><?php esc_attr_e( 'Oblique', 'netlink-plus' ); ?></option>
							<option value="inherit"<# if ( 'inherit' === data.value['font-style'] ) { #>selected<# } #>><?php esc_attr_e( 'Inherit', 'netlink-plus' ); ?></option>
						</select>
					</div>
				<# } #>
			</div>
			...
			<# if( !_.isUndefined( data.choices['text_transform'] ) ) { #>
				<div class="full-width">
					...
					<select class="text-transform">
						...
						<option value="none"<# if ( 'none' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_attr_e( 'None', 'netlink-plus' ); ?></option>
						<option value="capitalize"<# if ( 'capitalize' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_attr_e( 'Capitalize', 'netlink-plus' ); ?></option>
						<option value="uppercase"<# if ( 'uppercase' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_attr_e( 'Uppercase', 'netlink-plus' ); ?></option>
						<option value="lowercase"<# if ( 'lowercase' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_attr_e( 'Lowercase', 'netlink-plus' ); ?></option>
						<option value="initial"<# if ( 'initial' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_attr_e( 'Initial', 'netlink-plus' ); ?></option>
						<option value="inherit"<# if ( 'inherit' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_attr_e( 'Inherit', 'netlink-plus' ); ?></option>
					</select>
				</div>
			<# } #>
				...
				<# if( !_.isUndefined( data.choices['text_align'] ) ) { #>
					<div class="one-half">
						...
						<select class="text-align">
							...
							<option value="unset"<# if ( 'unset' === data.value['text-align'] ) { #>selected<# } #>><?php esc_attr_e( 'Unset', 'netlink-plus' ); ?></option>
							<option value="left"<# if ( 'left' === data.value['text-align'] ) { #>selected<# } #>><?php esc_attr_e( 'Left', 'netlink-plus' ); ?></option>
							<option value="center"<# if ( 'center' === data.value['text-align'] ) { #>selected<# } #>><?php esc_attr_e( 'Center', 'netlink-plus' ); ?></option>
							<option value="right"<# if ( 'right' === data.value['text-align'] ) { #>selected<# } #>><?php esc_attr_e( 'Right', 'netlink-plus' ); ?></option>
							<option value="justify"<# if ( 'justify' === data.value['text-align'] ) { #>selected<# } #>><?php esc_attr_e( 'Justify', 'netlink-plus' ); ?></option>
							<option value="inherit"<# if ( 'inherit' === data.value['text-align'] ) { #>selected<# } #>><?php esc_attr_e( 'Inherit', 'netlink-plus' ); ?></option>
						</select>
					</div>
				<# } #>
				...
				<# if( !_.isUndefined( data.choices['text_decoration'] ) ) { #>
					<div class="one-half">
						...
						<select class="text-decoration">
							...
							<option value="none"<# if ( 'none' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_attr_e( 'None', 'netlink-plus' ); ?></option>
							<option value="underline"<# if ( 'underline' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_attr_e( 'Underline', 'netlink-plus' ); ?></option>
							<option value="overline"<# if ( 'overline' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_attr_e( 'Overline', 'netlink-plus' ); ?></option>
							<option value="line-through"<# if ( 'line-through' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_attr_e( 'Line Through', 'netlink-plus' ); ?></option>
							<option value="inherit"<# if ( 'inherit' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_attr_e( 'Inherit', 'netlink-plus' ); ?></option>
						</select>
					</div>
				<# } #>
			</div>
