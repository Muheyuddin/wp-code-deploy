...
class Netlink_Customize_Control_Background extends WP_Customize_Control {
	...
	public function enqueue() {
		wp_enqueue_style( 'netlink-plus-background-control',  NETLINK_PLUS_DIR_URL.'customizer/controls/background/background.css', null, NETLINK_PLUS_VERSION );
		wp_enqueue_script( 'netlink-plus-background-control', NETLINK_PLUS_DIR_URL.'customizer/controls/background/background.js', array( 'jquery', 'customize-base' ), NETLINK_PLUS_VERSION, true );
		wp_localize_script( 'netlink-plus-background-control',
			'wdtPlusCustomizerControlBackground', array(
				'placeholder'  => esc_html__( 'No file selected', 'netlink-plus' ),
				'lessSettings' => esc_html__( 'Less Settings', 'netlink-plus' ),
				'moreSettings' => esc_html__( 'More Settings', 'netlink-plus' ),
			)
		);
	}
	...
	protected function content_template() {
		...
		<div class="background-wrapper">
			...
			<div class="background-color">
				<h4><?php esc_html_e( 'Background Color', 'netlink-plus' ); ?></h4>
			...
			<div class="background-image">
				<h4><?php esc_html_e( 'Background Image', 'netlink-plus' ); ?></h4>
				<div class="attachment-media-view background-image-upload">
					<# if ( data.value['background-image'] ) { #>
						<div class="thumbnail thumbnail-image"><img src="{{ data.value['background-image'] }}" /></div>
					<# } else { #>
						<div class="placeholder"><?php esc_html_e( 'No File Selected', 'netlink-plus' ); ?></div>
					<# } #>
					<div class="actions">
						<button class="button background-image-upload-remove-button<# if ( ! data.value['background-image'] ) { #> hidden <# } #>">
							<?php esc_attr_e( 'Remove', 'netlink-plus' ); ?>
						</button>
						<button type="button" class="button background-image-upload-button"><?php esc_attr_e( 'Select File', 'netlink-plus' ); ?></button>

						<# if ( data.value['background-image'] ) { #>
							<a href="javascript:void(0);" class="more-settings" data-direction="up">
								<span class="message"><?php esc_html_e( 'Less Settings', 'netlink-plus' ); ?></span>
								<span class="icon">↑</span>
							</a>
						<# } else { #>
							<a href="javascript:void(0);" class="more-settings" data-direction="down">
								<span class="message"><?php esc_html_e( 'More Settings', 'netlink-plus' ); ?></span>
								<span class="icon">↓</span>
							</a>
						<# } #>
					</div>
			...
			<div class="background-size">
				<h4><?php esc_html_e( 'Background Size', 'netlink-plus' ); ?></h4>
				<div class="buttonset">
					<input {{{ data.inputAttrs }}} class="switch-input screen-reader-text" type="radio" value="cover" name="_customize-bg-{{{ data.id }}}-size"
						...
						<label class="switch-label switch-label-<# if ( 'cover' === data.value['background-size'] ) { #>on <# } else { #>off<# } #>" for="{{ data.id }}cover"><?php esc_html_e( 'Cover', 'netlink-plus' ); ?></label>
					</input>
					<input {{{ data.inputAttrs }}} class="switch-input screen-reader-text" type="radio" value="contain" name="_customize-bg-{{{ data.id }}}-size"
						...
						<label class="switch-label switch-label-<# if ( 'contain' === data.value['background-size'] ) { #>on <# } else { #>off<# } #>" for="{{ data.id }}contain"><?php esc_html_e( 'Contain', 'netlink-plus' ); ?></label>
					</input>
					<input {{{ data.inputAttrs }}} class="switch-input screen-reader-text" type="radio" value="auto" name="_customize-bg-{{{ data.id }}}-size"
						...
						<label class="switch-label switch-label-<# if ( 'auto' === data.value['background-size'] ) { #>on <# } else { #>off<# } #>" for="{{ data.id }}auto"><?php esc_html_e( 'Auto', 'netlink-plus' ); ?></label>
					</input>
				</div>
			</div>
			...
			<div class="background-attachment">
				<h4><?php esc_html_e( 'Background Attachment', 'netlink-plus' ); ?></h4>
				<div class="buttonset">
					<input {{{ data.inputAttrs }}} class="switch-input screen-reader-text" type="radio" value="inherit" name="_customize-bg-{{{ data.id }}}-attachment" id="{{ data.id }}inherit" <# if ( 'inherit' === data.value['background-attachment'] ) { #> checked="checked" <# } #>>
						<label class="switch-label switch-label-<# if ( 'inherit' === data.value['background-attachment'] ) { #>on <# } else { #>off<# } #>" for="{{ data.id }}inherit"><?php esc_html_e( 'Inherit', 'netlink-plus' ); ?></label>
					</input>
					<input {{{ data.inputAttrs }}} class="switch-input screen-reader-text" type="radio" value="scroll" name="_customize-bg-{{{ data.id }}}-attachment" id="{{ data.id }}scroll" <# if ( 'scroll' === data.value['background-attachment'] ) { #> checked="checked" <# } #>>
						<label class="switch-label switch-label-<# if ( 'scroll' === data.value['background-attachment'] ) { #>on <# } else { #>off<# } #>" for="{{ data.id }}scroll"><?php esc_html_e( 'Scroll', 'netlink-plus' ); ?></label>
					</input>
					<input {{{ data.inputAttrs }}} class="switch-input screen-reader-text" type="radio" value="fixed" name="_customize-bg-{{{ data.id }}}-attachment" id="{{ data.id }}fixed" <# if ( 'fixed' === data.value['background-attachment'] ) { #> checked="checked" <# } #>>
						<label class="switch-label switch-label-<# if ( 'fixed' === data.value['background-attachment'] ) { #>on <# } else { #>off<# } #>" for="{{ data.id }}fixed"><?php esc_html_e( 'Fixed', 'netlink-plus' ); ?></label>
					</input>
				</div>
			</div>
