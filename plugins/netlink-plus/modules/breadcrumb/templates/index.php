...
if( !class_exists( 'NetlinkPlusBCTemplate' ) ) {
    class NetlinkPlusBCTemplate {
        ...
        function load_frontend() {
            add_filter( 'netlink_breadcrumb_params', array( $this, 'register_breadcrumb_params' ) );
            add_filter( 'netlink_breadcrumb_get_template_part', array( $this, 'register_template' ), 10, 2 );

            add_action( 'netlink_after_main_css', array( $this, 'enqueue_assets' ) );

            add_filter( 'netlink_header_wrapper_classes', array( $this, 'register_header_class' ), 20, 1 );
        }

        function register_header_class($classes) {

            $breadcrumb_position = netlink_customizer_settings('breadcrumb_position');
            $dark_bg_breadcrumb = netlink_customizer_settings('enable_dark_bg_breadcrumb');
        ...
        function enqueue_assets() {
            wp_enqueue_style( 'site-breadcrumb', NETLINK_PLUS_DIR_URL . 'modules/breadcrumb/assets/css/breadcrumb.css', NETLINK_PLUS_VERSION );
        }
        ...
        function base_meta_style( $meta ) {
            ...
            if( !empty( $bg_css ) && empty( $enable_overlay ) ) {
                ...
                if( !empty( $overlay_color ) || !empty( $overlay_gradient_color ) ) {
                    $bg_css = netlink_customizer_bg_settings( array( 'background-color' => $overlay_color, 'gradient-background-color' => $overlay_gradient_color, 'breadcrumb_overlay_bg_color' => true ) );
            ...
            if( !empty( $style ) ){
                wp_register_style( 'netlink-breadcrumb-inline', '', array (), NETLINK_PLUS_VERSION, 'all' );
                wp_enqueue_style( 'netlink-breadcrumb-inline' );
                wp_add_inline_style( 'netlink-breadcrumb-inline', $style );
            }
        }

        function register_breadcrumb_params() {

            $enable_delimiter = netlink_customizer_settings( 'change_breadcrumb_delimiter' );
            $delimiter        = netlink_customizer_settings( 'breadcrumb_delimiter' );

            $delimiter = ( $enable_delimiter ) ? '<span class="'.esc_attr($delimiter).'"></span>' : '<span class="breadcrumb-default-delimiter"></span>';

            $wrapper_class    = array();
            $enable_darkbg    = netlink_customizer_settings( 'enable_dark_bg_breadcrumb' );
            $breadcrumb_style = netlink_customizer_settings( 'breadcrumb_style' );
            ...
            $bc_overlay_bg_color = netlink_customizer_settings( 'breadcrumb_overlay_bg_color' );
            if($bc_overlay_bg_color) {
                $wrapper_class[] = 'overlay-wrapper';
            }

            $hide_content    = netlink_customizer_settings( 'hide_content' );
            if( $hide_content ) {
                $wrapper_class[] = 'hide-content';
            }

            $params = array(
                'home'             => esc_html__( 'Home', 'netlink-plus' ),
                'home_link'        => home_url('/'),
                'delimiter'        => $delimiter,
        ...
        function register_meta_params( $post_id ) {
            ...
            $post_meta = get_post_meta( $post_id, '_netlink_breadcrumb_settings', true );
        ...
        function register_template( $args, $post_id ) {
            ...
            if( empty($bc_meta) || ( array_key_exists( 'layout', $bc_meta ) && $bc_meta['layout'] != 'disable' ) ) {
                ...
                if( array_key_exists( 'layout', $bc_meta ) && $bc_meta['layout'] == 'individual-option' ) {
                    ...
                    $enable_bc = netlink_customizer_settings( 'enable_breadcrumb' );
                    if( ! $enable_bc ) {
                        return;
                    }
                }

                $bc_source = netlink_customizer_settings( 'breadcrumb_source' );

                switch( $bc_source ):

                    case 'default':
                    default:
                        netlink_template_part( 'breadcrumb', 'templates/default/title-content', '', $template_args );
                    break;
...
NetlinkPlusBCTemplate::instance();