...
class WeDesignTech_Widget_Base_Heading {
	...
	public function init_scripts() {
		return array (
			$this->name() =>  WEDESIGNTECH_ELEMENTOR_ADDON_DIR_URL.'inc/widgets/heading/assets/js/script.js'
		);
	}
	...
	public function create_elementor_controls($elementor_object) {
		...
		// Highlight Elements
			...
			$elementor_object->start_controls_section( 'wdt_section_highlight_elements', array(
				...
				$elementor_object->add_control( 'split_heading', array(
						'label'              => esc_html__( 'Split text', 'wdt-elementor-addon' ),
						'type'               => \Elementor\Controls_Manager::SWITCHER,
						'frontend_available' => true,
						'default'            => '',
						'separator'          => 'before',
		...
		if(isset($settings['title']) && !empty($settings['title'])) {
			$output .= '<'.esc_attr($settings['title_tag']).' class="wdt-heading-title-wrapper wdt-heading-align-'.esc_attr($settings['title_vertical_align']).' '.esc_attr($deco_class).'">';
				$output .= '<span class="wdt-heading-title">';
					...
					$output .= $this->render_title_splitup_html($settings['title'], $settings['colored_elements'], $settings['split_heading']);
					if(isset($settings['apply_decoration_to']) && $settings['apply_decoration_to'] == 'title') {
	...
	public function render_title_splitup_html($title, $colored_elements, $split_heading) {
		...
		$text = '';
			...
			foreach($splitted_titles as $key => $splitted_title) {
				...
				if(in_array($updated_key, $colored_elements_splitted)) {
					...
					if ( $split_heading  == 'true') {
						$text .= '<span class="wdt-split-heading-wrapper">';
							$splitting_titles = str_split($splitted_title);
							foreach( $splitting_titles as $key => $splitted_titles_values ) {
								$text .= '<span class="wdt-split-heading-title">'. $splitted_titles_values .'</span>';
							}
						$text .= '</span>';
					} else {
