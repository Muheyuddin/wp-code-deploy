	...
	public function render_form_html($settings) {
		...
		$output .= '<form class="wdt-mailchimp-subscribe-form '.esc_attr($button_class).'" name="mailchimpSubscribeForm" action="#" method="post">';
			...
			if($settings['terms_condition'] == 'yes') {
				$output .= '<div>';
					$output .= '<input type="checkbox" class="wdt-terms-and-conditions" required="required"><span class="wdt-terms-condition-lbl">' . esc_html__('I agree to receive this newsletter and know that I can unsubscribe at any time.', 'wdt-elementor-addon') . '</span>';
				$output .= '</div>';
			}
