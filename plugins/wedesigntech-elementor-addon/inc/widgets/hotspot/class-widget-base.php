...
class WeDesignTech_Widget_Base_Hotspot {
		...
		$elementor_object->start_controls_section( 'wdt_section_hotspots', array(
			...
			$repeater->start_controls_tabs( 'hotspot_items' );
				...
				$repeater->start_controls_tab(
					...
					$repeater->add_control(
						'hotspot_media_image_description',
						array(
							'type' => \Elementor\Controls_Manager::RAW_HTML,
							'raw'  => esc_html__( 'Image option will be used only when "Hotspot Type" is "Image".', 'wdt-elementor-addon' ),
							'content_classes' => 'elementor-descriptor'
						)
					);
							...
							$output .= '<div '.$widget_object->get_render_attribute_string( $hotspot_repeater_trigger_setting_key ).'>';
								...
								if($settings['hotspot_type'] == 'icon') {
									...
									$output .= '<div class="wdt-hotspot-item-number">';
										$output .= $key+1;
									$output .= '</div>';
								} elseif($settings['hotspot_type'] == 'image') {
									if($hotspot['hotspot_media_image']['url']) {										
										$image_item = array ();
										$image_item['hotspot_media_image'] = $hotspot['hotspot_media_image'];
										$image_item['hotspot_media_image_style'] = '';
										$image_item['hotspot_media_image_shape'] = '';
										$output .= '<div class="wdt-hotspot-item-image">';
											$output .= $this->render_image($hotspot, '', '');
										$output .= '</div>';
									}
								} else {
									$output .= '<div class="wdt-hotspot-item-default">';
	...
	public function render_image($item, $link_start, $link_end) {
		$output = '';
		if ( ! empty( $item['hotspot_media_image']['url'] ) ) :
			$class = '';
			$output .= '<div class="wdt-content-image-wrapper '.esc_attr($class).'">';
				$output .= '<div class="wdt-content-image">';

					$media_image_setting = array ();
					$media_image_setting['image'] = $item['hotspot_media_image'];
					$media_image_setting['image_size'] = 'thumbnail';
					$media_image_setting['image_custom_dimension'] = isset($item['hotspot_media_image_custom_dimension']) ? $item['hotspot_media_image_custom_dimension'] : array ();
